...
const OfferCardArc = ({ offer, onRestore, onDelete }) => (
  <article className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-[0_8px_24px_rgba(2,6,23,0.06)] dark:shadow-[0_8px_24px_rgba(2,6,23,0.6)] overflow-hidden hover:shadow-lg transform transition hover:-translate-y-1">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 flex flex-wrap gap-2 items-center">
            ...
            <span className="text-xs text-slate-400 dark:text-slate-300">• {offer.updated}</span>
          </div>
          ...
          <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex gap-2 flex-col sm:flex-row w-full sm:w-auto">
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-3 py-2 rounded-md bg-white dark:bg-slate-700 border text-sm hover:bg-white/90 dark:hover:bg-slate-700"
                aria-label={`Desarchivar ${offer.title}`}
              >
                <FontAwesomeIcon icon={faCirclePlus} className="text-slate-700 dark:text-slate-200" />
                Desarchivar
              </button>
    ...
    <main className="min-h-screen bg-gradient-to-b from-blue-50 via-white to-slate-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 py-8 text-slate-900 dark:text-slate-100">
      <div className="max-w-6xl mx-auto px-4">
        <div className="mb-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-slate-100">Ofertas Archivadas</h1>
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Puedes desarchivar, editar o eliminar ofertas.</p>
          </div>

          <div className="w-full md:w-auto flex flex-col sm:flex-row gap-3">
            <div className="relative flex items-center bg-white dark:bg-slate-800 rounded-lg shadow-sm px-3 py-2 w-full sm:w-72">
              <FontAwesomeIcon icon={faSearch} className="text-slate-400 dark:text-slate-300" />
              <input
                aria-label="Buscar ofertas archivadas"
                placeholder="Buscar título, categoría o descripción..."
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                className="ml-3 outline-none bg-transparent text-sm text-slate-700 dark:text-slate-200 w-full"
              />
            </div>
            ...
            <select
              ...
              className="bg-white dark:bg-slate-800 px-3 py-2 rounded-lg shadow-sm text-sm border dark:border-slate-700"
        ...
        <section className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {offers.length === 0 ? (
            <div className="col-span-full bg-white dark:bg-slate-800 rounded-2xl p-8 text-center shadow-sm">
              <p className="text-slate-600 dark:text-slate-300">No hay ofertas archivadas.</p>
            ...
            <div className="col-span-full bg-white dark:bg-slate-800 rounded-2xl p-8 text-center shadow-sm">
              <p className="text-slate-600 dark:text-slate-300">No se encontraron ofertas archivadas con esos criterios.</p>
              <div className="mt-4 flex justify-center">
                <Link to="/talentPage/actividades/ofertas/activas" className="text-blue-600 dark:text-blue-300 hover:underline">Ver ofertas activas</Link>
              </div>
            </div>
          ) : (
            filtered.map(offer => (
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="text-sm text-slate-500 dark:text-slate-300">
            Mostrando <span className="font-medium text-slate-700 dark:text-slate-200">{filtered.length}</span> ofertas archivadas
          </div>

          <div className="flex items-center gap-3">
            <Link to="/contratist-page/activities/offers/active" className="px-4 py-2 rounded-md bg-white dark:bg-slate-800 border dark:border-slate-700 hover:shadow-sm text-sm">Ver activas</Link>
