	...
	<article className="rounded-2xl shadow-md hover:shadow-2xl transition p-6 bg-white dark:bg-slate-800">
		<div className="flex flex-col sm:flex-row gap-4 sm:items-center">
			...
			<div className="flex-1 min-w-0">
				<h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100 truncate">{c.title}</h3>
				<p className="text-sm text-slate-600 dark:text-slate-300 mt-1">
					Cliente:{' '}
					<span className="font-medium text-slate-800 dark:text-slate-200">{c.client}</span>
				</p>
				...
				<div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
					...
					<div className="flex items-center gap-2 justify-start sm:justify-end text-slate-600 dark:text-slate-300">
						<div className="text-xs text-slate-500 dark:text-slate-400">Estado</div>
						<div
							className={`px-3 py-1 rounded-full text-sm font-medium ${
								c.status === 'En progreso' ? 'bg-emerald-600 text-white' : 'bg-yellow-500 text-white'
							}`}
						>
							{c.status}
						</div>
		...
		<div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
			<div className="flex gap-2 flex-wrap">
				...
				<button className="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-white dark:bg-slate-700 dark:text-slate-100 border dark:border-slate-700 text-sm hover:shadow-sm">
				...
				<header className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
					<div>
						...
						<p className="text-sm text-slate-600 dark:text-slate-300">
							Administra tus contratos activos y realiza acciones rápidas.
						</p>
					</div>

					<div className="flex gap-3">
						<Link
							to="/client-page/contracts/active"
							className="px-4 py-2 rounded-md bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold"
						>
							Activas
						</Link>
						<Link
							to="/client-page/contracts/history"
							className="px-4 py-2 rounded-md bg-white dark:bg-slate-800 border dark:border-slate-700 text-sm"
						>
							Historial
						</Link>
					</div>
				...
				<div className="grid gap-6">
					...
					{sampleActive.length === 0 && (
						<div className="col-span-full rounded-2xl p-8 text-center bg-white dark:bg-slate-800">
							<p className="text-slate-600 dark:text-slate-300">No tienes contratos activos.</p>
							<div className="mt-4">
								<Link
									to="/client-page/offers"
									className="inline-flex px-4 py-2 bg-emerald-600 text-white rounded-lg"
								>
									Ver ofertas
								</Link>
							</div>
